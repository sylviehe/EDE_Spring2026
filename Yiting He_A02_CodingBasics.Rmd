...
7.  Initial here to acknowledge that you did not use AI at all in completing this assignment: Yiting He___
...
#9
colnames(student_data) <- c("Name", "Score", "Scholarship")

```

10.  QUESTION: How is this data frame different from a matrix?

> Answer:A data frame can contain different types of data in different columns (e.g. numeric, character, logical), while a matrix can only contain one data type. In addition, data frames allow columns to have names and are more flexible for working with real-world data.
...
> Answer:The ifelse() function worked properly with the vector input, while if...else did not. This is because ifelse() is vectorized in R and can evaluate each element of a vector, whereas if...else is designed to work with a single logical condition.
